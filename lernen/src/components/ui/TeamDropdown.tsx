...
export function TeamDropdown() {
    ...
    return (
        <div className="relative">
            <button
                className="flex items-center space-x-2 text-gray-300 hover:text-white transition-colors duration-1000 text-lg"
                onClick={() => setIsOpen(!isOpen)}
                onBlur={() => setTimeout(() => setIsOpen(false), 200)}
            >
                <span>Meet the Team</span>
                <ChevronDown
                    className={`h-5 w-5 transition-transform duration-900 ease-in-out ${isOpen ? 'rotate-180' : ''
                        }`}
                />
            </button>
            ...
            >
                ...
                <div className="py-3">
                    {teamMembers.map((member, index) => (
                        <div
                            key={index}
                            className="px-6 py-4 hover:bg-gray-800/50 transition-colors duration-1000 cursor-pointer flex items-center space-x-5"
                            style={{
                        ...
                        >
                            <div className="flex-shrink-0">
                                <img
                                    src={member.image}
                                    alt={member.name}
                                    className="h-24 w-24 rounded-full object-cover border border-gray-800 transition-transform duration-1000 hover:scale-105"
                                />
                            </div>
